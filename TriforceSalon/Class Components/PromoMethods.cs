        ...
        public async Task AddPromo(Guna2DataGridView itemsDataGrid, long ID, DateTime start, DateTime end, string percentage, string name)
        {
            try
            {
                using(var conn = new MySqlConnection(mysqlcon))
                {
                    ...
                    string insertToPromoTable = "INSERT INTO salon_promos (PromoStart, PromoEnd, PromoCode, PromoName, PromoItemsID, DiscountPercent) " +
                        "VALUE(@dateStart, @dateEnd, @promoCode, @promoName, @PitemsID, @discount)";
